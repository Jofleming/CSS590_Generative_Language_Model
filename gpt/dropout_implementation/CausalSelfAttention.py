...
class CausalSelfAttention(Module):
    def __init__(self, emb_dim, num_heads, drop_rate=0.0):
        super().__init__()
        ...
        self.attn_drop = Dropout(drop_rate)
    ...
    def forward(self, x):
        ...
        self.softmax_out = scipy_special.softmax(scores, axis=-1).astype(x.dtype)
        self.attn_weights = self.attn_drop.forward(self.softmax_out)
    ...
    def backward(self, grad_output):
        ...
        grad_attn = self.attn_drop.backward(grad_attn)
        dot = xp.sum(grad_attn * self.softmax_out, axis=-1, keepdims=True)
        grad_scores = self.softmax_out * (grad_attn - dot)
