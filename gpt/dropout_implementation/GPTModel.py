...
class GPTModel(Module):
    def __init__(self, vocab_size, block_size, emb_dim, num_heads, num_layers, drop_rate=0.0):
          super().__init__()
          self.vocab_size = vocab_size
          self.block_size = block_size
          self.tok_emb  = Embedding(vocab_size, emb_dim)
          self.pos_emb  = Embedding(block_size, emb_dim)
          self.blocks   = Sequential([TransformerBlock(emb_dim, num_heads, drop_rate) for _ in range(num_layers)])
